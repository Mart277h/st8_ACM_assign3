...
for (n in seq(nrow(sim_data_simB))) {
  ...
  sim_data_simB$choice[n] <- round(sim_data_simB$belief[n], 1) #need to make a choice, should we sample or just take the straight value? 
}

  # which one should we do here, because its not rbinom when it is not binary data, have to choose from a normal distribution, cannot figure it out in my head right now
