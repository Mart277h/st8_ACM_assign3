...
```{r}
draws_weighted_p86 <- as_draws_df(samples_weighted_sc$draws())
```

```{r means}
ggplot(draws_weighted_p86) +
...
```{r looping through each participant for both models}
#creating a list for a for loop to go through, modelling by each participant
participant_list <- unique(df$Participant)

draws_simple <- tibble()

draws_weighted <- tibble()
...
for (i in participant_list){ 
  ...
  #adding the draws to the dataframe
  temp_df_w <- as_draws_df(samples_weighted$draws())
  
  temp_df_w <- temp_df_w %>% 
    mutate(subject = i)
  
  if (exists("draws_weighted")) {draws_weighted <- rbind(draws_weighted, temp_df_w)} #remember to clean if reusing
  else{draws_weighted <- temp_df_w}
  ...
  #adding the draws to the dataframe
  temp_df_s <- as_draws_df(samples_simple$draws())
  
  temp_df_s <- temp_df_s %>% 
    mutate(subject = i)
  
  if (exists("draws_simple")) {draws_simple <- rbind(draws_simple, temp_df_s)} #remember to clean if reusing
  else{draws_simple <- temp_df_s}
  
  
  #getting model weights for each of the models to compare 
  loo_simple <- samples_simple$loo(save_psis = TRUE)
  
  loo_weighted <- samples_weighted$loo(save_psis = TRUE)
  
    # model weights
  temp_weights <- loo_model_weights(list(loo_simple, loo_weighted))
  
  temp_weights_df <- tibble(subject = i, SimpleBayes = temp_weights[1], WeightedBayes = temp_weights[2])
  
  weights_df <- rbind(weights_df, temp_weights_df)
